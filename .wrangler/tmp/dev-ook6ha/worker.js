...
// .wrangler/tmp/bundle-A2F9Py/checked-fetch.js
...
async function requireAuthenticatedUser(c) {
  ...
  if (!SUPABASE_URL) {
    log.warn("[auth] SUPABASE_URL n\xE3o definido");
    return false;
  }
...
async function checkRateLimit(c, bucket, perMinute) {
...
function normalizeConfidencePercent(raw2) {
  if (typeof raw2 !== "number" || Number.isNaN(raw2)) return null;
  return raw2 <= 1 ? raw2 * 100 : raw2;
}
__name(normalizeConfidencePercent, "normalizeConfidencePercent");
...
async function requireAiswebRateLimit(c) {
  return checkRateLimit(c, `aisweb-public:${clientIp(c)}`, 60);
}
__name(requireAiswebRateLimit, "requireAiswebRateLimit");
...
__name(timingSafeTokenMatch, "timingSafeTokenMatch");
async function checkInternalAuth(c) {
...
var TIPOS_VALIDOS = ["INFRAERO", "DECEA", "POUSO"];
var DEMO_TIPO_CONTEXT = {
  INFRAERO: "um demonstrativo de Tarifa INFRAERO (tarifas aeroportu\xE1rias cobradas pela INFRAERO)",
  DECEA: "um demonstrativo de Tarifa DECEA (tarifas de navega\xE7\xE3o a\xE9rea cobradas pelo DECEA)",
  POUSO: "um demonstrativo de Tarifa de Pouso"
};
var DEMO_OCR_SYSTEM_PROMPT = /* @__PURE__ */ __name((tipo) => `Voc\xEA \xE9 um especialista em leitura de demonstrativos financeiros da avia\xE7\xE3o civil brasileira.
O documento enviado \xE9 ${DEMO_TIPO_CONTEXT[tipo]}.

Extraia os dados e responda APENAS com um objeto JSON v\xE1lido, sem markdown, sem texto antes ou depois, no seguinte formato exato:

{
  "tipo": "${tipo}",
  "numero_documento": string ou null,
  "competencia": string ou null (formato MM/YYYY se dispon\xEDvel),
  ...
  "itens": [
    {
      ...
      "operacao": string ou omitido (ex: c\xF3digo ICAO do aer\xF3dromo, quando a linha s\xF3 cita um local),
      "origem": string ou omitido (c\xF3digo ICAO de origem do trecho \u2014 preencha quando o documento listar origem e destino separados, o que \xE9 comum em demonstrativos DECEA),
      "destino": string ou omitido (c\xF3digo ICAO de destino do trecho),
      "matricula": string ou omitido (matr\xEDcula da aeronave daquela linha, se vier por linha),
      "valor": number
    }
  ]
}
...
Regras importantes:
- Extraia TODAS as linhas/opera\xE7\xF5es listadas no documento, uma por item em "itens", mesmo que o documento tenha v\xE1rias p\xE1ginas.
- Se a linha trouxer origem e destino separados, preencha "origem" e "destino" (n\xE3o preencha "operacao" nesse caso). Se trouxer s\xF3 um aer\xF3dromo, preencha "operacao" e omita origem/destino.
- Valores monet\xE1rios devem ser n\xFAmeros (ponto decimal, sem "R$", sem separador de milhar).
- Inclua itens com valor 0 (ex: opera\xE7\xF5es isentas) \u2014 n\xE3o os omita.
- "valor_total" deve ser o total exatamente como impresso no documento. Se n\xE3o houver um total expl\xEDcito, use null.
- Se um campo n\xE3o existir no documento, use null (campos do topo) ou omita a chave (campos opcionais dos itens).
- Nunca invente dados que n\xE3o estejam vis\xEDveis no documento.
- Responda SOMENTE com o JSON, nada mais.`, "DEMO_OCR_SYSTEM_PROMPT");
function extractJsonFromText(text) {
  const cleaned = text.trim().replace(/^```json\s*/i, "").replace(/^```\s*/i, "").replace(/```\s*$/i, "");
  const start = cleaned.indexOf("{");
  const end = cleaned.lastIndexOf("}");
  if (start === -1 || end === -1) throw new Error("Resposta da IA n\xE3o cont\xE9m JSON v\xE1lido");
  try {
    return JSON.parse(cleaned.slice(start, end + 1));
  } catch (err) {
    throw new Error(`JSON inv\xE1lido na resposta da IA (poss\xEDvel truncamento): ${err.message}`);
  }
}
__name(extractJsonFromText, "extractJsonFromText");
var SUPPORTED_IMAGE_TYPES = /* @__PURE__ */ new Set(["image/jpeg", "image/png", "image/gif", "image/webp"]);
var SUPPORTED_DOC_TYPES = /* @__PURE__ */ new Set(["application/pdf"]);
...
function buildContentBlock(dataBase64, mimeType) {
  ...
  throw new Error(`mimeType n\xE3o suportado: "${mimeType}" (use image/jpeg, image/png, image/gif, image/webp ou application/pdf)`);
}
__name(buildContentBlock, "buildContentBlock");
var CLAUDE_MODEL = "claude-sonnet-5";
async function callClaudeExtraction(c, dataBase64, mimeType, tipo) {
  const { ANTHROPIC_API_KEY } = c.env;
  if (!ANTHROPIC_API_KEY) throw new Error("ANTHROPIC_API_KEY n\xE3o configurada");
  const contentBlock = buildContentBlock(dataBase64, mimeType);
  const request = /* @__PURE__ */ __name(async (maxTokens) => {
    const controller = new AbortController();
          ...
          messages: [
            {
              ...
              content: [
                contentBlock,
                { type: "text", text: "Extraia os dados deste demonstrativo conforme as instru\xE7\xF5es." }
              ]
            }
          ]
        })
  ...
  if (data?.stop_reason === "max_tokens") {
    ...
    if (data?.stop_reason === "max_tokens") {
      throw new Error("Demonstrativo grande demais para extrair em uma \xFAnica chamada \u2014 considere dividir em p\xE1ginas menores");
    }
  }
  const textBlock = data?.content?.find((b) => b.type === "text");
  if (!textBlock?.text) throw new Error("Resposta da IA sem conte\xFAdo de texto");
  return extractJsonFromText(textBlock.text);
}
__name(callClaudeExtraction, "callClaudeExtraction");
function normalizeDemoResult(raw2, tipo) {
  const itensRaw = Array.isArray(raw2?.itens) ? raw2.itens : [];
  const itens = itensRaw.map((it) => ({
    data: String(it?.data ?? ""),
    hora: it?.hora ? String(it.hora) : void 0,
    operacao: it?.operacao ? String(it.operacao) : void 0,
    origem: it?.origem ? String(it.origem).toUpperCase() : void 0,
    destino: it?.destino ? String(it.destino).toUpperCase() : void 0,
    matricula: it?.matricula ? String(it.matricula).toUpperCase() : void 0,
    valor: typeof it?.valor === "number" ? it.valor : parseFloat(String(it?.valor ?? "").replace(",", "."))
  })).filter((it) => it.data && !Number.isNaN(it.valor));
  const somaItens = itens.reduce((s, i) => s + i.valor, 0);
  const valorTotalExtraido = typeof raw2?.valor_total === "number" ? raw2.valor_total : null;
  const valorTotal = valorTotalExtraido ?? (itens.length ? somaItens : null);
  const divergencia = valorTotalExtraido != null && itens.length ? Math.abs(valorTotalExtraido - somaItens) : 0;
  return {
    tipo,
    numero_documento: raw2?.numero_documento ?? null,
    competencia: raw2?.competencia ?? null,
    data_faturamento: raw2?.data_faturamento ?? null,
    aeronave_matricula: raw2?.aeronave_matricula ?? null,
    cliente_nome: raw2?.cliente_nome ?? null,
    valor_total: valorTotal,
    itens,
    _meta: {
      soma_itens: itens.length ? Math.round(somaItens * 100) / 100 : null,
      precisa_revisao_manual: divergencia > 0.05
      // mais de 5 centavos de diferença
    }
  };
}
__name(normalizeDemoResult, "normalizeDemoResult");
var RATEIO_SEM_MATCH = "N\xE3o Identificado / Traslado";
...
__name(brDateToIso, "brDateToIso");
async function buscarResponsavelLancamento(c, item) {
  const { SUPABASE_URL, SUPABASE_ANON_KEY } = c.env;
  if (!SUPABASE_URL || !SUPABASE_ANON_KEY) throw new Error("Supabase n\xE3o configurado (SUPABASE_URL/SUPABASE_ANON_KEY)");
...
__name(buscarResponsavelLancamento, "buscarResponsavelLancamento");
async function montarRelatorioRateio(c, itens) {
  const relatorio = [];
...
__name(montarRelatorioRateio, "montarRelatorioRateio");
app.get("/", (c) => c.text("ShareBrasil API \u{1F680}"));
async function valuationHandler(c) {
  try {
...
__name(valuationHandler, "valuationHandler");
...
app.get("/api/weather/:icao", async (c) => {
  if (!await requireAiswebRateLimit(c)) return c.json({ error: "Limite de requisi\xE7\xF5es atingido, tente novamente em instantes" }, 429);
...
app.get("/api/templates", async (c) => {
  const authOk = await requireAuthenticatedUser(c) || await checkInternalAuth(c);
  if (!authOk) return c.json({ error: "N\xE3o autorizado" }, 401);
  try {
...
app.post("/api/demonstrativo-ocr", async (c) => {
  try {
    ...
    if (!allowed) return c.json({ error: "Limite de requisi\xE7\xF5es atingido, tente novamente em instantes" }, 429);
    const body = await c.req.json();
    const { imageBase64, mimeType, tipo } = body;
    if (!imageBase64 || !mimeType) return c.json({ error: "imageBase64 e mimeType s\xE3o obrigat\xF3rios" }, 400);
    if (!tipo || !TIPOS_VALIDOS.includes(tipo)) {
      return c.json({ error: "tipo inv\xE1lido (use INFRAERO, DECEA ou POUSO)" }, 400);
    }
    const raw2 = await callClaudeExtraction(c, imageBase64, mimeType, tipo);
    const result = normalizeDemoResult(raw2, tipo);
...
app.post("/api/demonstrativo-rateio", async (c) => {
  try {
    ...
    const tipo = body.tipo ?? "DECEA";
    if (!imageBase64 || !mimeType) return c.json({ error: "imageBase64 e mimeType s\xE3o obrigat\xF3rios" }, 400);
    if (!TIPOS_VALIDOS.includes(tipo)) {
      return c.json({ error: "tipo inv\xE1lido (use INFRAERO, DECEA ou POUSO)" }, 400);
    }
    const raw2 = await callClaudeExtraction(c, imageBase64, mimeType, tipo);
    const demonstrativo = normalizeDemoResult(raw2, tipo);
...
app.post("/api/hotel-reservation", async (c) => {
  try {
    ...
    if (!allowed) return c.json({ error: "Limite de solicita\xE7\xF5es atingido, tente novamente em instantes" }, 429);
    const body = await c.req.json();
    const required = [
      "hotelId",
      "dataCheckin",
    ...
    if (missing.length) {
      return c.json({ error: `Campos obrigat\xF3rios faltando: ${missing.join(", ")}` }, 400);
    }
    if (!Number.isInteger(body.quantidadeHospedes) || body.quantidadeHospedes < 1) {
      return c.json({ error: "quantidadeHospedes deve ser um n\xFAmero inteiro maior que zero" }, 400);
    }
    if (!c.env.SUPABASE_URL || !c.env.SUPABASE_SERVICE_ROLE_KEY || !c.env.RESEND_API_KEY || !c.env.EMAIL_FROM) {
      log.error("[hotel-reservation] configura\xE7\xE3o de integra\xE7\xE3o ausente");
      return c.json({ error: "Servi\xE7o de reservas indispon\xEDvel" }, 503);
    }
    ...
    if (!hotel) return c.json({ error: "Hotel n\xE3o encontrado" }, 404);
    const destinatario = hotel.email_reservas || hotel.email;
    if (!destinatario) return c.json({ error: "Hotel n\xE3o possui email cadastrado para reservas" }, 422);
    const emailResponse = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from: c.env.EMAIL_FROM,
        to: [destinatario],
        reply_to: body.hospedeEmail || void 0,
        subject: `Solicita\xE7\xE3o de reserva \u2014 ${body.hospedeNome} (${body.dataCheckin} a ${body.dataCheckout})`,
        html: `
          <style>.hotel-reservation-note { font-size: 12px; color: #666; }</style>
          <h2>Nova solicita\xE7\xE3o de reserva</h2>
          ...
          <p><strong>H\xF3spede:</strong> ${escapeHtml(body.hospedeNome)}</p>
          ...
          <p><strong>N\xBA de h\xF3spedes:</strong> ${escapeHtml(body.quantidadeHospedes)}</p>
          ${body.observacoes ? `<p><strong>Observa\xE7\xF5es:</strong> ${escapeHtml(body.observacoes)}</p>` : ""}
          <hr>
          <p class="hotel-reservation-note">Solicita\xE7\xE3o gerada automaticamente pelo sistema Share Brasil.</p>
        `
      })
    });
    if (!emailResponse.ok) {
      log.error("[hotel-reservation] falha ao enviar email:", emailResponse.status);
      return c.json({ error: "Falha ao enviar solicita\xE7\xE3o por email" }, 500);
    }
    ...
    const reservas = await reservationResponse.json();
    return c.json({ success: true, reserva: reservas[0] ?? null });
  } catch (error) {
    log.error("[hotel-reservation]", error?.message ?? error);
    return c.json({ error: "Falha ao processar solicita\xE7\xE3o de reserva" }, 500);
  }
});
...
// .wrangler/tmp/bundle-A2F9Py/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-A2F9Py/middleware-loader.entry.ts
