INSERT INTO "sqlite_master"("sql") VALUES('CREATE TABLE _cf_KV (
    ...
    url_avatar TEXT
)'), ('CREATE TABLE assinaturas_email (id TEXT PRIMARY KEY NOT NULL, usuario_id TEXT NOT NULL UNIQUE, nome TEXT NOT NULL, cargo TEXT, telefone TEXT, endereco TEXT, email TEXT NOT NULL, logo_url TEXT, criado_em TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, atualizado_em TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP)'), ('CREATE TABLE auditoria_financeira (
  id TEXT PRIMARY KEY NOT NULL,
...
INSERT INTO "sqlite_master"("sql") VALUES('CREATE TABLE "contas_apagar" (
  ...
  criado_em TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
  atualizado_em TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP
)'), ('CREATE TABLE ctm_despesas_motor (
  id TEXT PRIMARY KEY,
  aeronave_id TEXT NOT NULL REFERENCES aeronave(id) ON DELETE CASCADE,
  data TEXT NOT NULL,
...
INSERT INTO "sqlite_master"("sql") VALUES('CREATE TABLE ctm_execucoes (
  id TEXT PRIMARY KEY,
  ...
  criado_em TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
  atualizado_em TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP
)'), ('CREATE TABLE ctm_mapa_componente (
  id TEXT PRIMARY KEY,
  aeronave_id TEXT NOT NULL REFERENCES aeronave(id) ON DELETE CASCADE,
  nome TEXT NOT NULL,
...
INSERT INTO "sqlite_master"("sql") VALUES('CREATE TABLE ctm_pecas_trocadas (
  id TEXT PRIMARY KEY,
  ordem_servico_id TEXT NOT NULL REFERENCES ctm_ordem_acompanhamento_servico(id) ON DELETE CASCADE,
  descricao TEXT NOT NULL,
  p_n_removido TEXT,
...
INSERT INTO "sqlite_master"("sql") VALUES('CREATE TABLE documentos_colaboradores (
    id TEXT PRIMARY KEY DEFAULT (lower(hex(randomblob(16)))),
    empresa_id TEXT NOT NULL,
    user_id TEXT NOT NULL,
...
INSERT INTO "sqlite_master"("sql") VALUES('CREATE TABLE financeiro_vinculos (
  id TEXT PRIMARY KEY NOT NULL,
...
INSERT INTO "sqlite_master"("sql") VALUES('CREATE TABLE lancamentos (
  id TEXT PRIMARY KEY NOT NULL,
...
INSERT INTO "sqlite_master"("sql") VALUES('CREATE TABLE notas_fiscais_saida (
    id TEXT PRIMARY KEY NOT NULL,
    numero TEXT NOT NULL,
...
INSERT INTO "sqlite_master"("sql") VALUES('CREATE TABLE recibos (
  ...
  -- Pagador: pode ser "empresa" ou "cotista_aeronave"
  pagador_tipo TEXT NOT NULL CHECK (pagador_tipo IN (''empresa'',''cotista_aeronave'')),
  pagador_id TEXT NOT NULL, -- id da empresa ou do cotista_aeronave, conforme pagador_tipo
  nome_pagador TEXT,
  ...
  valor INTEGER NOT NULL DEFAULT 0, -- valor em centavos (ex: R$ 1.234,56 = 123456)
  descricao TEXT,
  data_emissao TEXT,
  data_vencimento TEXT,
  forma_pagamento TEXT,
  ...
  categoria_movimentacao_id TEXT NOT NULL, -- referencia categoria_movimentacao_share (se tipo_caixa=''share'') ou categoria_movimentacao_cliente (se tipo_caixa in (''cliente'',''hold''))
  grupo_categoria TEXT,
...
INSERT INTO "sqlite_master"("sql") VALUES('CREATE INDEX idx_aeronave_id
...
ON ctm_ficha_peso_balanceamento (peso_balanceamento_id)'), ('CREATE INDEX idx_financeiro_fila_status ON financeiro_fila(status, criado_em)'), ('CREATE INDEX idx_folhas_pagamento_empresa_competencia
    ON folhas_pagamento (empresa_id, competencia_ano DESC, competencia_mes DESC)');
INSERT INTO "sqlite_master"("sql") VALUES('CREATE INDEX idx_folhas_pagamento_status
...
ON user_profiles(departamentos_email)'), ('CREATE INDEX idx_usuarios_empresas_empresa
    ON usuarios_empresas (empresa_id, perfil, status)');
INSERT INTO "sqlite_master"("sql") VALUES('CREATE INDEX idx_usuarios_empresas_usuario
