...
import { calculateDistanceNM, calculateNightTime, getSolarTimes, parseDMSCoordinate, } from './lib/flight-calculations';
import { getAirportCoordinates, searchAirports } from './lib/airports';
import financialRouter from './routes/financial';
const router = Router();
// ============= USERS ROUTES =============
// GET all users (cached - 10 min)
router.get('/users', cacheMiddleware({ ttl: 10 * 60 * 1000, key: 'users:all' }), async (req, res) => {
    try {
...
router.get('/users/:id', cacheMiddleware({
    ttl: 10 * 60 * 1000,
...
router.get('/users/:id/profile', cacheMiddleware({
    ttl: 10 * 60 * 1000,
...
router.get('/flights/:id', cacheMiddleware({
    ttl: 2 * 60 * 1000,
...
router.get('/flights/active/now', async (req, res) => {
    try {
        const { data, error } = await supabase
            .from('flight_schedules')
            .select(`
...
router.get('/clients', cacheMiddleware({ ttl: 15 * 60 * 1000, key: 'clients:all' }), async (req, res) => {
    try {
        const { data, error } = await supabase
            .from('clients')
            .select('*')
...
router.get('/clients/:id', cacheMiddleware({
    ttl: 15 * 60 * 1000,
...
router.get('/clients/:id/contracts', cacheMiddleware({
    ttl: 24 * 60 * 60 * 1000,
...
router.get('/aircraft', cacheMiddleware({ ttl: 20 * 60 * 1000, key: 'aircraft:all' }), async (req, res) => {
    try {
        const { data, error } = await supabase
            .from('aircraft')
            .select('*')
...
router.get('/aircraft/:id', cacheMiddleware({
    ttl: 10 * 60 * 1000,
...
router.get('/aircraft/:id/availability', cacheMiddleware({
    ttl: 5 * 60 * 1000,
...
router.get('/aerodromes', cacheMiddleware({ ttl: 24 * 60 * 60 * 1000, key: 'aerodromes:all' }), async (req, res) => {
    try {
        const { data, error } = await supabase
            .from('aerodromes')
...
router.get('/aerodromes/details', cacheMiddleware({ ttl: 24 * 60 * 60 * 1000, key: 'aerodromes:details' }), async (req, res) => {
    try {
        const { data, error } = await supabase
            .from('aerodromes')
            .select('*')
...
router.get('/aerodromes/:icao', cacheMiddleware({
    ttl: 24 * 60 * 60 * 1000,
...
router.get('/categories', cacheMiddleware({ ttl: 12 * 60 * 60 * 1000, key: 'categories:all' }), async (req, res) => {
    try {
        const { data, error } = await supabase
...
router.get('/categories/unique-by-type', cacheMiddleware({ ttl: 12 * 60 * 60 * 1000, key: 'categories:unique' }), async (req, res) => {
    try {
        const { data, error } = await supabase
            .from('categorias_movimentacao')
            .select('id, nome, tipo')
...
router.post('/flight-documents', async (req, res) => {
    try {
 ...
 * }
 */
router.post('/flight-calculations', async (req, res) => {
    try {
        const { departureIcao, arrivalIcao, arrivalManual, flightDate, landingTime, } = req.body;
        // Validações básicas
        ...
        // Calcular distância
        const distance = calculateDistanceNM(departureParsed.lat, departureParsed.lng, arrivalLat, arrivalLng);
...
router.get('/weather/metar', cacheMiddleware({
    ttl: 10 * 60 * 1000,
    key: (req) => `metar:${req.query.icao?.toUpperCase() || 'unknown'}`
}), async (req, res) => {
    try {
...
router.post('/logbook', async (req, res) => {
    try {
        const { total_time, time, night_time, entry_date, departure_aerodrome, arrival_aerodrome, ac_time, cor_time, dep_time, pou_time, pic_canac, sic_canac, client_id, aircraft_id, pousos, fuel_added, distance_nm, flight_nature, ifr_time, observations, } = req.body;
        if (total_time <= 0) {
            return res.status(400).json({
...
router.post('/consolidacao/consolidar-rateio', async (req, res) => {
    try {
        const { lancamento_rateio_id, bank_transaction_id, bank_reconciliation_id, horas_voadas } = req.body;
...
router.post('/consolidacao/consolidar-horas-mensais', async (req, res) => {
    try {
        const { cliente_id, aeronave_id, ano, mes, horas_voadas } = req.body;
...
router.get('/consolidacao/extrato-cliente/:cliente_id', async (req, res) => {
    try {
...
router.get('/consolidacao/resumo-mensal-cliente/:cliente_id', async (req, res) => {
    try {
        const { cliente_id } = req.params;
...
router.get('/consolidacao/comparativo-uso/:aeronave_id', async (req, res) => {
    try {
...
router.get('/consolidacao/pendencias-cliente/:cliente_id', async (req, res) => {
    try {
        const { cliente_id } = req.params;
...
router.get('/consolidacao/analise-anual/:cliente_id', async (req, res) => {
    try {
        const { cliente_id } = req.params;
        const { ano } = req.query;
        let query = supabase
...
router.get('/consolidacao/status-conciliacao', async (req, res) => {
    try {
        const defaultStatus = {
            pendentes: 0,
            conciliadas: 0,
...
router.get('/consolidacao/reembolsos-pendentes', async (req, res) => {
    try {
        const { data, error } = await supabase
