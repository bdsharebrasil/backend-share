...
export async function ensureFinanceKernel(db: Database) {
  ...
  await db.prepare(`CREATE TABLE IF NOT EXISTS contas_apagar (id TEXT PRIMARY KEY NOT NULL, data_vencimento TEXT NOT NULL, data_pagamento TEXT, valor_centavos INTEGER NOT NULL CHECK (valor_centavos > 0), valor REAL, categoria_id TEXT, categoria_nome TEXT, descricao TEXT, aeronave_id TEXT, fornecedor_id TEXT, cotista_id TEXT, boleto_url TEXT, nf_url TEXT, lancamento_id TEXT NOT NULL UNIQUE, conta_bancaria_id TEXT, colaborador_id TEXT, status TEXT NOT NULL DEFAULT 'EM_ABERTO' CHECK (status IN ('EM_ABERTO','PENDENTE','PAGO','ATRASADO','CANCELADO')), comprovante_pagamento_url TEXT, banco_pagamento TEXT, criado_por TEXT, criado_em TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, atualizado_em TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, origem_tipo TEXT, origem_id TEXT, idempotency_key TEXT)`).run()
  await db.prepare(`CREATE TABLE IF NOT EXISTS contas_areceber (id TEXT PRIMARY KEY NOT NULL, data_vencimento TEXT, data_recebimento TEXT, data_pagamento TEXT, valor_centavos INTEGER NOT NULL CHECK (valor_centavos > 0), valor REAL, categoria_id TEXT, categoria_nome TEXT, descricao TEXT, aeronave_id TEXT, cotista_id TEXT, holding_id TEXT, socio_id TEXT, lancamento_receita_id TEXT, lancamento_cliente_id TEXT, lancamento_id TEXT, lancamentos_id TEXT, nf_saida_id TEXT, status TEXT NOT NULL DEFAULT 'EM_ABERTO' CHECK (status IN ('EM_ABERTO','PENDENTE','RECEBIDO','ATRASADO','CANCELADO')), banco_recebimento TEXT, comprovante_recebimento_url TEXT, criado_por TEXT, criado_em TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, atualizado_em TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, origem_tipo TEXT, origem_id TEXT, idempotency_key TEXT)`).run()
...
export async function createExpense(db: Database, body: Row, userId: string | null) {
  ...
  const status = direct || Boolean(body.pago ?? body.data_pagamento) ? 'PAGO' : 'EM_ABERTO'
