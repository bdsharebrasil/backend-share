...
async function isColaboradorManager(c: Context<{ Bindings: Bindings }>, user: Colaborador): Promise<boolean> {
  const result = await c.env.SHARE_DB.prepare("SELECT 1 FROM usuarios_funcoes WHERE user_id = ?1 AND lower(replace(replace(trim(funcao), ' ', '_'), '-', '_')) IN ('admin', 'financeiro_master', 'gestor_master', 'rh_master', 'rh') LIMIT 1").bind(user.id).first()
  return Boolean(result)
}
