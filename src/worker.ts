...
// Middleware - CORS configurado para aceitar do Pages
app.use('*', cors({
  origin: '*', // Em produção, especifique: 'https://your-project.pages.dev'
...
const fetchAISWebMETAR = async (icao: string): Promise<any> => {
  try {
    ...
    console.log(`[AISWeb METAR] ✅ Dados recebidos para ${icao}:`, data);
    return data;
  } catch (error) {
...
app.get('/api/weather/metar/:icao', async (c) => {
  try {
    ...
    console.log(`[METAR] 📥 Requisição recebida para: ${icao}`);
    
    // Validate ICAO
    if (!isValidICAO(icao)) {
      console.error(`[METAR] ❌ ICAO inválido: ${icao}`);
      return c.json({ 
    ...
    const result = await getCached(c, `metar:${icao}`, 300, async () => {
      ...
      if (!aiswebData || !aiswebData.rawOb) {
        console.error(`[METAR] ❌ Sem dados METAR para ${icao}`);
        return { 
      ...
      console.log(`[METAR] ✅ Dados processados para ${icao}`);
    ...
    console.error(`[METAR] 💥 Erro fatal:`, error);
    return c.json({ 
