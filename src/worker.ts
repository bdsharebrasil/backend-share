...
async function garantirColunasLancamentoEnvio(c: Context<{ Bindings: Bindings }>) {
  const db = portalDb(c)
  for (const coluna of ['periodicidade TEXT', 'anexos_json TEXT DEFAULT \'[]\'']) await db.prepare(`ALTER TABLE lancamentos ADD COLUMN ${coluna}`).run().catch(() => undefined)
}
...
app.get('/api/financeiro/notas-saida', async c => { const user=await shareBrasilUser(c); if(!user) return c.json({error:'nao_autorizado'},401); await garantirTabelasNfSaida(c); const db=portalDb(c); const [notas,recibos]=await Promise.all([db.prepare(`SELECT n.*,COALESCE(cl.razao_social,hs.nome) cliente_nome,cl.cnpj cliente_cnpj,a.matricula_registro aeronave_matricula,ca.contas_areceber_id FROM notas_fiscais_saida n LEFT JOIN cotista_aeronave ca ON ca.id=n.cotista_aeronave_id LEFT JOIN cliente cl ON cl.id=n.cliente_id LEFT JOIN hold_socios hs ON hs.id=n.socio_id LEFT JOIN aeronave a ON a.id=n.aeronave_id ORDER BY n.data_criacao DESC`).all(),db.prepare(`SELECT r.*,COALESCE(cl.razao_social,hs.nome) cliente_nome,cl.cnpj cliente_cnpj,a.matricula_registro aeronave_matricula,ca.aeronave_id,ca.cliente_id,ca.socio_id,ar.id contas_areceber_id FROM recibos_saida r LEFT JOIN cotista_aeronave ca ON ca.id=r.cotista_id LEFT JOIN cliente cl ON cl.id=r.cliente_id LEFT JOIN hold_socios hs ON hs.id=r.socio_id LEFT JOIN aeronave a ON a.id=r.aeronave_id LEFT JOIN contas_areceber ar ON ar.id=r.contas_areceber_id ORDER BY r.data_emissao DESC`).all()]); return c.json({notas:notas.results,recibos:recibos.results}) })
